...
const AttendanceRecord = require("../models/AttendanceRecord");
const User = require("../models/User");
const Location = require("../models/Location"); // Will use the Polygon-based Location model
const Timetable = require("../models/Timetable");

// --- Configuration ---
const CHECKIN_WINDOW_MINUTES = 10; // Allow check-in 10 mins after start
const CHECKOUT_WINDOW_MINUTES = 5; // Allow check-out 5 mins after end
// LOCATION_CHECK_RADIUS_METERS_BUFFER has been removed as it's specific to circular geofences
...
// The isWithinRadius function is removed as it's not used for polygon geofencing.
// If you need generic distance calculation for other purposes, keep it or use a library.

// --- Populate Helper ---
const populateRecordFields = [
  { path: "subject", select: "subjectCode name _id" },
  { path: "professor", select: "firstName lastName email _id" },
  { path: "location", select: "name building _id" }, // Still relevant for polygon locations
];
...
exports.manualCheckIn = async (req, res) => {
  ...
  try {
    ...
    const classLocation = await Location.findById(locationId).lean();
    if (
      !classLocation ||
      !classLocation.geofence ||
    ...
    // Geo-validation using Polygon $geoIntersects
    const studentGeoPoint = {
      type: "Point",
      coordinates: [numLongitude, numLatitude], // GeoJSON: [longitude, latitude]
    };

    const isInsideGeofence = await Location.findOne({
      _id: classLocation._id,
      geofence: {
        $geoIntersects: {
          $geometry: studentGeoPoint,
        },
      },
    }).lean(); // Use .lean() for performance if only checking existence

    const geoPassed = !!isInsideGeofence;
    console.log(
      `[GeoCheck - Polygon] Student at [${studentGeoPoint.coordinates.join(
        ", "
      )}]. Inside geofence for location ${classLocation.name} (${
        classLocation._id
      }): ${geoPassed}`
    );
    ...
    const checkInData = {
      ...
      locationAccuracy: accuracy,
      coordinates: { type: "Point", coordinates: [numLongitude, numLatitude] },
      detectedBSSIDs: Array.isArray(detectedBSSIDs) ? detectedBSSIDs : [],
    };
    ...
    if (error.name === "ValidationError" && error.errors) {
      // More specific Mongoose validation error check
...
exports.manualCheckOut = async (req, res) => {
  ...
  try {
    ...
    console.log(
      `[GeoCheck - Polygon] Student at [${studentGeoPoint.coordinates.join(
        ", "
      )}] for checkout. Inside geofence for location ${classLocation.name} (${
        classLocation._id
      }): ${geoPassed}`
    );
    ...
    // Ensure professor and subject are populated before creating room name
    if (!populatedRecord.professor || !populatedRecord.subject) {
      console.error(
        "Error: Professor or Subject not populated for socket room."
      );
      // Potentially handle this error or proceed without socket if critical info missing
    }
...
// --- Get student's current session status ---
exports.getStudentCurrentSessionStatus = async (req, res) => {
  const studentId = req.user._id;
  const { subjectId, classDateStr, scheduledStartTime, term } = req.query;
...
exports.getStudentAttendanceHistory = async (req, res) => {
  const studentId = req.user._id;
  const { subjectId, term, fromDate, toDate, page = 1, limit = 10 } = req.query;
  ...
  if (fromDate || toDate) {
    filter.classDate = {};
    if (fromDate)
      filter.classDate.$gte = new Date(
        new Date(fromDate).setUTCHours(0, 0, 0, 0)
      ); // Normalize to start of day UTC
    if (toDate)
      filter.classDate.$lte = new Date(
...
exports.getStudentAttendanceSummary = async (req, res) => {
  ...
  try {
    const studentObjectId = mongoose.Types.ObjectId(studentId); // Ensure it's an ObjectId
    ...
    const aggregationPipeline = [
      {
        $match: {
          ...
          term: term,
          status: { $in: ["Present", "Late", "Absent", "Excused"] }, // Consider Excused as well if needed for total count
        },
      },
      {
        $group: {
      ...
      {
        $unwind: { path: "$subjectInfo", preserveNullAndEmptyArrays: true }, // Preserve if subject somehow deleted
      },
      {
        $project: {
          _id: 0,
          ...
          subjectName: "$subjectInfo.name",
          presentCount: 1,
          totalClasses: "$totalClassesMarked",
          attendancePercentage: {
            $cond: [
              { $eq: ["$totalClassesMarked", 0] },
              0,
...
exports.getProfessorSessionAttendance = async (req, res) => {
  ...
  try {
    ...
    const sessionAttendance = await AttendanceRecord.find(filter)
      ...
      .select("student status checkIn.timestamp checkOut.timestamp createdAt")
      .sort({ "student.lastName": 1, "student.firstName": 1 })
      .lean();
    // TODO: Augment with expected students (e.g., from User collection based on timetable criteria)
    res.status(200).json(sessionAttendance);
  } catch (error) {
    console.error("Get Professor Session Attendance Error:", error);
    res
...
exports.updateAttendanceStatus = async (req, res) => {
  ...
  try {
    ...
    // Authorization: Admin can modify any, Professor can only modify their own classes
    if (userRole !== "admin" && !record.professor.equals(userId)) {
      return res
        .status(403)
    ...
    record.adminOverrideReason = reason
      ? `Changed from ${oldStatus} to ${newStatus} by ${userRole} (${
          req.user.email
        }) on ${new Date().toISOString()}: ${reason}`
      : `Status changed from ${oldStatus} to ${newStatus} by ${userRole} (${
          req.user.email
        }) on ${new Date().toISOString()}.`;

    // If changing to 'Absent', clear checkIn and checkOut data if desired
    // if (newStatus === 'Absent') {
    ...
    if (io && oldStatus !== newStatus) {
      if (!populatedRecord.professor || !populatedRecord.subject) {
        console.error(
          "Error: Professor or Subject not populated for socket room in status update."
        );
      } else {
        const classDateObjectForRoom = populatedRecord.classDate;
        const formattedDate = classDateObjectForRoom
          .toISOString()
          .split("T")[0];
        const room = `session-${populatedRecord.professor._id}-${populatedRecord.subject._id}-${formattedDate}-${populatedRecord.scheduledStartTime}`;
        io.to(room).emit("attendanceUpdate", {
...
exports.getAdminAnalytics = async (req, res) => {
  const { term } = req.query;
  if (!term)
    return res.status(400).json({ message: "Term is required for analytics." });

  try {
    const pipeline = [
      {
        $match: { term: term, status: { $in: ["Present", "Late", "Absent"] } },
      }, // Can include 'Excused' based on policy
      {
        $group: {
          _id: null,
          presentCount: {
