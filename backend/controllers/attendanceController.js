...
function isWithinRadius(pointCoords, centerCoords, radiusMeters) {
  ...
  console.log(
    `[GeoCheck] Distance: ${distance.toFixed(
      2
    )}m, Required Radius (with buffer): ${radiusMeters}m, Classroom Radius: ${
      radiusMeters - LOCATION_CHECK_RADIUS_METERS_BUFFER
    }m`
  );
...
exports.manualCheckIn = async (req, res) => {
  ...
  try {
    // --- Time Window Check ---
    const checkinEndTime = addMinutesToTime(
      scheduledStartTime,
    ...
    if (
      ...
      !classLocation.location.coordinates
    ) {
      return res
        .status(404)
        .json({ message: "Classroom location details not found." });
    }

    // --- Perform Validations ---
    const deviceIdMatch =
      !!user.boundDeviceId && user.boundDeviceId === deviceId;
    const geoPassed = isWithinRadius(
      [longitude, latitude],
      classLocation.location.coordinates,
      classLocation.radiusMeters + LOCATION_CHECK_RADIUS_METERS_BUFFER
    );
    const wifiPassed =
      classLocation.trustedWifiBSSIDs &&
      classLocation.trustedWifiBSSIDs.length > 0
        ? classLocation.trustedWifiBSSIDs.some((trustedBssid) =>
            detectedBSSIDs.includes(trustedBssid)
          )
        : true;
    ...
    if (existingRecord) {
      ...
      console.log(
        `Existing record found (status: ${existingRecord.status}), updating check-in and setting to Pending.`
      );
    ...
    if (!populatedRecord) {
      ...
      console.error(
        "Error: Record was saved/updated but could not be found for population immediately after."
      );
      return res
        .status(500)
        .json({ message: "Internal server error after processing check-in." });
    }

    if (io) {
      io.to(room).emit("attendanceUpdate", {
        type: actionTypeForSocket,
        studentId: populatedRecord.student,
    ...
    res
      .status(500)
      .json({ message: "Server error during check-in.", error: error.message });
  }
};
...
exports.manualCheckOut = async (req, res) => {
  ...
  if (
    !attendanceRecordId ||
    !mongoose.Types.ObjectId.isValid(attendanceRecordId)
  ) {
    return res
      .status(400)
      .json({ message: "Valid attendanceRecordId is required for check-out." });
  }
  if (
    !currentTimeStr ||
    latitude == null ||
    ...
    return res
      .status(400)
      .json({ message: "Missing required fields for check-out validation." });
  }

  try {
    const recordToUpdate = await AttendanceRecord.findOne({
    ...
    const checkoutEndTime = addMinutesToTime(
      recordToUpdate.scheduledEndTime,
    ...
    if (!user)
      return res
        .status(404)
        .json({ message: "Student not found (internal error)." });

    const classLocation = await Location.findById(
      recordToUpdate.location
    ).lean(); // Get location from record
    if (
      !classLocation ||
      !classLocation.location ||
      !classLocation.location.coordinates
      ...
      return res
        .status(404)
        .json({
          message: "Classroom location details not found for validation.",
        });
    }
    ...
    if (recordToUpdate.checkIn) {
      // Should be true if status was 'Pending'
      recordToUpdate.status = "Present";
    } else {
      console.warn(
        `Check-out performed for record ${recordToUpdate._id} without a prior check-in. Status remains ${recordToUpdate.status}.`
      );
    }
    ...
    if (!populatedRecord) {
      console.error(
        "Error: Record was saved for checkout but could not be found for population."
      );
      return res
        .status(500)
        .json({ message: "Internal server error after processing check-out." });
    }

    const formattedDateForRoom = populatedRecord.classDate
      .toISOString()
      .split("T")[0];
...
exports.updateAttendanceStatus = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: `Attendance status updated to ${newStatus}.`, record });
  } catch (error) {
    console.error("Update Attendance Status Error:", error);
    res
      .status(500)
      .json({ message: "Server error updating attendance status." });
  }
};
...
exports.getStudentAttendanceHistory = async (req, res) => {
  ...
  try {
    ...
    const records = await AttendanceRecord.find(filter)
      ...
      .select(
        "-checkIn.detectedBSSIDs -checkIn.deviceReportedId -checkOut.detectedBSSIDs -checkOut.deviceReportedId"
      ) // Be more specific
      .lean();
...
exports.getStudentAttendanceSummary = async (req, res) => {
  ...
  if (!term) {
    return res
      .status(400)
      .json({ message: "Term query parameter is required for summary." });
  }
  ...
  try {
            ...
            $cond: [
              ...
              0, // Percentage is 0 if no classes marked
              {
                $round: [
                  {
                    $multiply: [
                      { $divide: ["$presentCount", "$totalClassesMarked"] },
                      100,
                    ],
                  },
                  0,
    ...
    res
      .status(500)
      .json({ message: "Server error fetching attendance summary." });
  }
};
...
exports.getProfessorSessionAttendance = async (req, res) => {
  ...
  const { subjectId, locationId, classDateStr, scheduledStartTime, term } =
    req.query;
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Server error fetching session attendance." });
  }
};
...
exports.updateAttendanceStatus = async (req, res) => {
  ...
  try {
    ...
    record.status = newStatus;
    record.adminOverrideReason = reason
      ? `Changed by ${userRole} (${
          req.user.email
        }) on ${new Date().toISOString()}: ${reason}`
      : `Status changed to ${newStatus} by ${userRole} (${
          req.user.email
...
exports.getAdminAnalytics = async (req, res) => {
  ...
  try {
    ...
    if (!term)
      return res
        .status(400)
        .json({ message: "Term is required for analytics." });
...
/**
 * @desc    Professor updates the location for a specific class session
 * @route   PUT /api/attendance/session/update-location
 * @access  Private (Professor)
 */
exports.updateSessionLocation = async (req, res) => {
  const professorId = req.user._id; // Logged-in professor
  const io = req.io;
  const {
    subjectId,
    // originalLocationId, // Good for logging, but not strictly needed for query if other params are present
    classDateStr,
    scheduledStartTime,
    term,
    newLocationId, // The ID of the newly selected location
  } = req.body;
  ...
  if (
    ...
    return res
      ...
      .json({
        message:
          "Missing required fields (subjectId, classDateStr, scheduledStartTime, term, newLocationId).",
      });
  }
  if (
    !mongoose.Types.ObjectId.isValid(subjectId) ||
    ...
    return res
      .status(400)
      .json({ message: "Invalid ID format for subjectId or newLocationId." });
  }
  ...
  try {
    ...
    // 1. Fetch the new location details to broadcast
    const newLocation = await Location.findById(newLocationId).lean();
    if (!newLocation) {
      return res
        .status(404)
        .json({ message: "The selected new location was not found." });
    }

    // 2. Update any existing AttendanceRecords for this specific session
    // This is important if students have already checked in or are pending.
    const updateResult = await AttendanceRecord.updateMany(
      {
        subject: subjectId,
        professor: professorId, // Ensure only the professor teaching this session can change it
        classDate: classDateObject,
        scheduledStartTime: scheduledStartTime,
        term: term,
      },
      { $set: { location: newLocationId } } // Update the location ref
    );

    console.log(
      `Updated ${updateResult.modifiedCount} existing attendance records for session location change.`
    );

    // 3. Construct Socket.IO room name and event payload
    // Room name should be consistent across client and server
    // Example: session-room-SUBJECTID-YYYY-MM-DD-HH:MM-TERM
    const roomName = `session-room-${subjectId}-${classDateStr}-${scheduledStartTime}-${term.replace(
      /\s+/g,
      "_"
    )}`; // Sanitize term for room name
    ...
    const eventPayload = {
      ...
      term: term,
      newLocation: {
        // Send details of the new location
        id: newLocation._id.toString(),
        name: newLocation.name,
        building: newLocation.building,
        // Optionally include coordinates if client needs them directly for map updates
        // coordinates: newLocation.location.coordinates,
        // radiusMeters: newLocation.radiusMeters
      },
    };

    // 4. Emit the event to the specific session room
    if (io) {
      io.to(roomName).emit("sessionLocationUpdated", eventPayload);
      console.log(
        `Emitted 'sessionLocationUpdated' to room '${roomName}' with payload:`,
        eventPayload
      );
    } else {
      console.warn(
        "Socket.IO instance (req.io) not available. Cannot emit 'sessionLocationUpdated'."
      );
    }

    res.status(200).json({
      message: `Session location successfully updated to '${newLocation.name}'. ${updateResult.modifiedCount} existing records affected.`,
      updatedLocation: eventPayload.newLocation,
    });
  } catch (error) {
    ...
    res
      .status(500)
      .json({
        message: "Server error while updating session location.",
        error: error.message,
      });
  }
};
