...
const handleRegistration = async (res, userData, roleSpecificData, role, deviceId) => {
  try {
    ...
    if (error.code === 11000 && error.keyPattern && error.keyPattern.boundDeviceId) {
         return res.status(400).json({ message: "This device is already linked to another account." });
    }
    if (error.code === 11000) { // Handle other duplicate keys
...
const handleLogin = async (req, res, expectedRole) => {
  ...
  try {
    ...
    const user = await User.findOne({ email }).select("+password +boundDeviceId");  // *** Fetch user WITH boundDeviceId selected ***
    ...
    if (user.boundDeviceId) {
      ...
      try {
           ...
           if (deviceAlreadyBound) {
               ...
               return res.status(409).json({ message: "Login failed: This device is linked to a different account." }); // 409 Conflict
           }
           ...
           if (saveError.code === 11000) { // Double check for race condition duplicate
